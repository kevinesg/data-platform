...
| `deploy-prod` | [deploy-prod.yml](deploy-prod.yml) | Manual dispatch with `prod` environment approval | Promotes all five QA image-manifest entries to prod, validates prod dbt compile, recreates the prod Airflow stack, and verifies the healthy complete DAG inventory. |
