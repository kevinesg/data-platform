...
| `clickhouse-ci` | [clickhouse-ci.yml](clickhouse-ci.yml) | Pull requests and pushes to `main` that change the ClickHouse component, plus manual dispatch | Validates the pinned ClickHouse Compose contract with temporary non-secret paths and credentials; it does not start or publish a production service. |
...
| `deploy-qa` | [deploy-qa.yml](deploy-qa.yml) | Manual dispatch | Deploys the selected Git ref with the latest matching immutable data-platform images and the immutable private ETL image resolved from its QA candidate pointer, waits for service health, and verifies the complete DAG inventory. |
| `deploy-prod` | [deploy-prod.yml](deploy-prod.yml) | Manual dispatch with `prod` environment approval | Promotes all four QA image-manifest entries to prod, validates prod dbt compile, recreates the prod Airflow stack, and verifies the healthy complete DAG inventory. |
